...
class GestureMouseController:
    def __init__(self):
        try:
            ...
            st.error(f"Error loading Haar Cascade: {e}. Please ensure OpenCV is installed.")
        ...
        self.head_tilt_left = 0
        self.head_tilt_right = 0

    def calibrate(self, face):
        (x, y, w, h) = face
        face_center_x = x + w // 2
        self.head_tilt_left = face_center_x - 100  # Adjust threshold as needed
        self.head_tilt_right = face_center_x + 100  # Adjust threshold as needed
    ...
    def detect_click_gestures(self, face, gray_frame):
        ...
        if not self.calibrated:
            self.calibrate(face)
        if face_center_x < self.head_tilt_left:
            if current_time - self.last_click_time > self.click_cooldown:
                try:
                    pyautogui.rightClick()
                    st.toast("Head Tilt Left - Right Click!")
                    self.last_click_time = current_time
                except Exception as e:
                    st.error(f"Right-click error: {e}")
        elif face_center_x > self.head_tilt_right:
            if current_time - self.last_click_time > self.click_cooldown:
                try:
                    pyautogui.rightClick()
                    st.toast("Head Tilt Right - Right Click!")
                    self.last_click_time = current_time
                except Exception as e:
                    st.error(f"Right-click error: {e}")
